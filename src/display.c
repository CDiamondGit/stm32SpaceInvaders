...
static void drawLineLowSlope(uint16_t x0,
                             uint16_t y0,
                             uint16_t x1,
                             uint16_t y1,
                             uint16_t Colour);
static void drawLineHighSlope(uint16_t x0,
                              uint16_t y0,
                              uint16_t x1,
                              uint16_t y1,
                              uint16_t Colour);
...
void initSPI(void) {
  ...
  // enable SSM, set SSI, enable SPI, PCLK/2, MSB First Master, Clock = 1 when
  // idle
  SPI1->CR1 =
      (1 << 9) + (1 << 8) + (1 << 6) + (1 << 2) + (1 << 1) +
      (1 << 0);  // Might get away with removing bit 3 here and get 24MHz clock
...
void printTextBold(const char* Text,
                   uint16_t x,
                   uint16_t y,
                   uint16_t ForeColour,
                   uint16_t BackColour) {
  // Bold effect achieved by drawing each column twice (pixel doubling
  // horizontally) This effectively makes the font width double: FONT_WIDTH * 2
  uint8_t Index = 0;
  uint8_t Row, Col;
  const uint8_t* CharacterCode = 0;
  uint16_t TextBox[(FONT_WIDTH * 2) * FONT_HEIGHT];  // Double width buffer
  uint16_t len;
  ...
  for (Index = 0; Index < len; Index++) {
    ...
    while (Col < FONT_WIDTH) {
      ...
      while (Row < FONT_HEIGHT) {
        uint16_t colour =
            (CharacterCode[Col] & (1 << Row)) ? ForeColour : BackColour;

        // Write each pixel twice side-by-side (columns Col*2 and Col*2+1)
        TextBox[(Row * (FONT_WIDTH * 2)) + (Col * 2)] = colour;
        TextBox[(Row * (FONT_WIDTH * 2)) + (Col * 2) + 1] = colour;

        Row++;
      }
      Col++;
    }
    putImage(x, y, FONT_WIDTH * 2, FONT_HEIGHT, (uint16_t*)TextBox, 0, 0);
    x = x + (FONT_WIDTH * 2) + 2;
  }
}
...
void printText(const char* Text,
               ...
               uint16_t ForeColour,
               uint16_t BackColour) {
  // This function draws each character individually.  It uses an array called
  // TextBox as a temporary storage
  // location to hold the dots for the character in question.  It constructs the
  // image of the character and then calls on putImage to place it on the screen
  uint8_t Index = 0;
  uint8_t Row, Col;
  const uint8_t* CharacterCode = 0;
...
void printTextX2(const char* Text,
                 uint16_t x,
                 uint16_t y,
                 uint16_t ForeColour,
                 uint16_t BackColour) {
#define Scale 2
  // This function draws each character individually scaled up by a factor of 2.
  // It uses an array called TextBox as a temporary storage
  // location to hold the dots for the character in question.  It constructs the
  // image of the character and then calls on putImage to place it on the screen
  uint8_t Index = 0;
  uint8_t Row, Col;
  ...
  for (Index = 0; Index < len; Index++) {
    ...
    while (Col < FONT_WIDTH) {
      ...
      while (Row < FONT_HEIGHT) {
        if (CharacterCode[Col] & (1 << Row)) {
          TextBox[((Row * Scale) * FONT_WIDTH * Scale) + (Col * Scale)] =
              ForeColour;
          TextBox[((Row * Scale) * FONT_WIDTH * Scale) + (Col * Scale) + 1] =
              ForeColour;
          TextBox[(((Row * Scale) + 1) * FONT_WIDTH * Scale) + (Col * Scale)] =
              ForeColour;
          TextBox[(((Row * Scale) + 1) * FONT_WIDTH * Scale) + (Col * Scale) +
                  1] = ForeColour;
        } else {
    ...
    putImage(x, y, FONT_WIDTH * Scale, FONT_HEIGHT * Scale, (uint16_t*)TextBox,
             0, 0);
...
void printNumber(uint16_t Number,
                 ...
                 uint16_t ForeColour,
                 uint16_t BackColour) {
  // This function converts the supplied number into a character string and then
  // calls on puText to
...
void printNumberX2(uint16_t Number,
                   uint16_t x,
                   uint16_t y,
                   uint16_t ForeColour,
                   uint16_t BackColour) {
...
void drawLineLowSlope(uint16_t x0,
                      uint16_t y0,
                      uint16_t x1,
                      uint16_t y1,
                      uint16_t Colour) {
...
void drawLineHighSlope(uint16_t x0,
                       uint16_t y0,
                       uint16_t x1,
                       uint16_t y1,
                       uint16_t Colour) {
